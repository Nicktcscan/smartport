...
export default function SADDeclaration() {
  ...
  // PDF/report generator using browser print (dependency-free)
  const generateSADReport = async (s) => {
    try {
      ...
      const html = `
        <html>
          ...
          <body>
            <h1>SAD ${s.sad_no} — Discharge Report</h1>
            <div class="meta">
              <div><strong>Regime:</strong> ${s.regime || '—'}</div>
              <div><strong>Declared weight:</strong> ${declared.toLocaleString()} kg</div>
              <div><strong>Recorded weight:</strong> ${recorded.toLocaleString()} kg</div>
            ...
            <div style="margin-top:18px;">
              <strong>Notes:</strong>
              <p class="small">This report is generated by the SAD management UI. Use your browser's Print dialog to save as PDF.</p>
            </div>
          </body>
        </html>
      `;
      ...
      if (!w) {
        toast({ title: 'Popup blocked', description: 'Please allow popups for this site to generate PDF', status: 'warning' });
        return;
      }
      w.document.open();
      ...
      // small delay to let browser render before print; user can save as PDF
      setTimeout(() => {
        try { w.print(); } catch (e) { /* ignore */ }
      }, 500);

      toast({ title: 'Report opened', description: 'A printable report window opened. Use Print -> Save as PDF.', status: 'info', duration: 6000 });
      await pushActivity(`PDF report generated for ${s.sad_no}`);
    } catch (err) {
      console.error('generateSADReport', err);
      toast({ title: 'Could not generate report', description: err?.message || 'Unexpected', status: 'error' });
    }
  };
  ...
  // Prevent accidental page refresh/submit when editing inline:
  // while editingSadId is set, suppress Enter key default behavior across the document.
  useEffect(() => {
    const handler = (e) => {
      if (editingSadId && e.key === 'Enter') {
        e.preventDefault();
        // Keep focus — user must click Save to persist
      }
    };
    ...
    <Container maxW="8xl" py={6}>
      ...
      <Box bg="white" p={4} borderRadius="md" boxShadow="sm" mb={6}>
        <Flex gap={3} align="center" wrap="wrap">
          ...
          <Select size="sm" placeholder="Filter by status" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} maxW="160px">
            <option value="">All</option>
            {SAD_STATUS.map(st => <option key={st} value={st}>{st}</option>)}
          </Select>

          <Select size="sm" placeholder="Filter by regime" value={regimeFilter} onChange={(e) => setRegimeFilter(e.target.value)} maxW="200px">
            <option value="">All</option>
                      ...
                      <Td isNumeric>
                        {editingSadId === s.sad_no ? (
                          <Input
                            size="sm"
                            type="number"
                            value={editData.declared_weight ?? 0}
                      ...
                      <Td>
                        <HStack>
                          {editingSadId === s.sad_no ? (
                            ...
                            <Menu>
                              <MenuButton as={IconButton} type="button" aria-label="Actions" icon={<FaEllipsisV />} size="sm" />
                              <MenuList>
                                ...
                                <MenuItem icon={<FaFilePdf />} onClick={() => generateSADReport(s)}>Generate PDF Report</MenuItem>
                                <MenuDivider />
