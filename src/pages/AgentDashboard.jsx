  ...
  const mapOutgateRow = (row, ticketSubmittedAt = null) => {
    ...
    return {
      ...
      data: {
        ...
        tare: row.tare ?? null,
        net: row.net ?? null,
        driver: row.driver ?? row.driver_name ?? 'N/A',
    ...
    <Box p={{ base: 4, md: 8 }}>
      <Flex justify="space-between" align="center" mb={6} gap={4} flexWrap="wrap">
        <Box>
          ...
          <Text color="gray.500">Search outgate (confirmed exits) by SAD → filter by date/time/truck → export or print. Results include Pending (tickets) and Exited (outgate).</Text>
        </Box>
